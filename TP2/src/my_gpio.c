...
// Private functions
static my_gpio_pins_t
gpioGetPin (my_gpio_map_t const pin) {
	return (my_gpio_pins_t)&(gpio_pins_init[pin]);
}

static bool
checkRange (my_gpio_map_t const pin) {
	return (pin < 0) || (pin > MAX_GPIO_PINS);
}

static uint32_t
pinToBit (uint8_t pin) {
	return (1 << pin);
}
...
my_gpio_init (my_gpio_map_t const pin,
	...
	switch (config) {
		...
		case MY_GPIO_INPUT:
			Chip_SCU_PinMux (mcu_pin->scu_port,
			                 mcu_pin->scu_pin,
			                 SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS,
			                 mcu_pin->func);
			Chip_GPIO_SetDir (LPC_GPIO_PORT,
			                  mcu_pin->gpio_port,
			                  pinToBit (mcu_pin->gpio_pin),
			                  MY_GPIO_INPUT);
			break;
...
my_gpio_read (my_gpio_map_t const pin) {
	...
	my_gpio_pins_t mcu_pin = gpioGetPin (pin);
	tmp_val = Chip_GPIO_ReadPortBit (LPC_GPIO_PORT,
	                                 mcu_pin->gpio_port,
	                                 mcu_pin->gpio_pin);
